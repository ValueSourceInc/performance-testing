...
export function completenessMarkdown(aws, meta, client, before, after, analysis, issues = []) {
  ...
  const rows = groups.map(group => {
    ...
    const reference = metrics.filter(m => metricSummary(m, start, end, { includePartial: true }).count > 0).length;
    return [group, !metrics.length ? '未采集/未发现资源' : `${observed}/${metrics.length} 条指标有严格窗口内完整分钟；${reference}/${metrics.length} 条有相交分钟（含边界参考）`];
  });
  ...
  return '## 报告完整性\n\n' + table(['检查项', '本轮证据'], rows) + '\n\n' +
    ...
    '数据缺失不表示资源空闲；AWS 指标按 60 秒统计桶返回，严格统计只使用完整分钟，边界分钟单列参考，不计入严格统计。错误类 CloudWatch 指标可能在无错误时不发布，逐项覆盖率见第 8 节。提前停止的未结束请求不等同服务端失败。\n\n';
}
...
export function metricSummary(metric, start, end, options = {}) {
  const period = metric.period * 1000;
  const valid = Number.isFinite(start) && Number.isFinite(end) && end > start && period > 0;
  const values = metric.points.filter(([time, value]) => {
    const at = Date.parse(time);
    const overlaps = options.includePartial ? at < end && at + period > start : at >= start && at + period <= end;
    return valid && Number.isFinite(value) && overlaps;
  }).map(([, value]) => value);
  const sum = values.reduce((a, b) => a + b, 0);
  const expected = valid
    ? options.includePartial
      ? Math.max(0, Math.ceil(end / period) - Math.floor(start / period))
      : Math.max(0, Math.floor(end / period) - Math.ceil(start / period))
    : 0;
  return { count: values.length, average: values.length ? sum / values.length : null,
    ...
    sum: values.length ? sum : null, expected };
}
...
export function monitoringMarkdown(aws, meta, client, before, after) {
  ...
  if (!aws) text += 'AWS 指标未采集。使用 `bash run-aws.sh smoke` 接通自动采集。\n\n';
    ...
    text += 'CloudWatch 原始粒度为 60 秒；严格统计仅使用完整落在窗口内的分钟桶。缺失不按零处理；边界分钟单列参考，覆盖窗口外时间，不能归属于单一档位、并入档内 Sum 或还原秒级尖峰。\n\n';
    const start = Date.parse(meta.planStartedAt || meta.createdAt), end = Date.parse(meta.finishedAt);
    ...
    function windowTables(first, last, label = '严格窗口统计') {
      let output = `#### ${label}\n\n` + table(headers, rows(first, last)) + '\n\n';
      const boundary = [];
      if (Number.isFinite(first) && Number.isFinite(last) && last > first) {
        for (const metric of series) for (const [time, value] of metric.points) {
          const at = Date.parse(time), until = at + metric.period * 1000;
          if (Number.isFinite(value) && at < last && until > first && (at < first || until > last)) {
            boundary.push([metric.group, metric.resource, metric.metric, metric.statistic, metric.unit,
              time, new Date(until).toISOString(), formatValue(value, metric.unit)]);
          }
        }
      }
      output += '#### 边界分钟参考（不计入严格统计，不跨档相加）\n\n';
      return output + (boundary.length
        ? table(['组件', '资源', '指标', '分钟统计', '单位', '桶开始', '桶结束', '整桶值'], boundary)
        : '无可用边界分钟；不表示零。') + '\n\n';
    }
    text += '### 测试全程（含排空）\n\n' + windowTables(start, end) + '\n\n';

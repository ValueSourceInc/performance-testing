...
export function analyze(records, plan, runEndMs) {
  ...
  const reasons = {};
  for (const r of records) {
    const key = JSON.stringify([r.window, r.model, r.stream, r.promptBucket ?? null]);
  ...
  return { totals: summarize(records),
    ...
    groups: [...groups.values()].map(rs => ({ window: rs[0].window, bucket: rs[0].promptBucket ?? null, model: rs[0].model, stream: rs[0].stream, ...summarize(rs) })),

...
export function execChat(body, tags = {}) {
  ...
  const bucket = body.promptBucket ?? null;
  const metricTags = { model: body.model, stream: String(!!body.stream), ...tags,
    window: window?.id || 'outside_plan', phase: window?.phase || 'unknown', ...(bucket ? { bucket } : {}) };
  issued.add(1, metricTags);
  // 请求体字节数: 真实样本/长短 prompt 的流量与解析开销对账依据
  // k6 无 Buffer,手算 UTF-8 字节长度;纯 ASCII(合成 sized 样本)直接取 length,
  // 8M 字符的逐码点循环是 longstream 每 VU 的主要 CPU 开销之一
  ...
  const bodyBytes = /[^\x00-\x7F]/.test(payload) ? utf8Bytes(payload) : payload.length;
  record({ kind: 'start', id, window: metricTags.window, model: body.model, stream: !!body.stream, promptBucket: bucket,

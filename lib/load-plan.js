...
export function buildPlan(name, env = {}) {
  ...
  let scenario;
  if (name === 'stress') {
    const peak = positive(env.STRESS_MAX_VUS ?? env.STRESS_PEAK_VUS, 6000);
    const start = positive(env.STRESS_START_VUS, 2000);
    if (peak < start) throw new Error('STRESS_MAX_VUS must be >= STRESS_START_VUS');
    const targets = [...new Set(Array.from({ length: 5 }, (_, i) => Math.round(start + (peak - start) * i / 4)))];
    ...
    if (durationMs(env.STRESS_RECOVERY_DURATION || '0s') > 0) {
      ramp('recovery', targets[targets.length - 1], targets[0], env.STRESS_RAMP_DURATION || '10s');
      hold('recovered', targets[0], env.STRESS_RECOVERY_DURATION);
    }
    ...
    const vus = positive(env.MIXED_VUS, 2000);
    const duration = env.MIXED_DURATION || '3m';
    hold('mixed', vus, duration);
    scenario = { executor: 'constant-vus', vus, duration };
  } else if (name === 'longstream') {
    // 长流场景: 固定并发、全流式,时长形态见 scenarios/longstream.js 注释
    const vus = positive(env.LONG_VUS, 2000);
    const duration = env.LONG_DURATION ?? env.SOAK_DURATION ?? '5m';
  ...
  return { name, mode: 'concurrency', durationMs: at, windows, scenario };
}

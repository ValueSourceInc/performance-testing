...
if (resultsIdx > 0) {
  const endIdx = thresholdsIdx > resultsIdx && thresholdsIdx > 0 ? thresholdsIdx : lines.length;
  const metricsLines = lines.slice(resultsIdx + 1, endIdx).filter(l => l.trim() && !l.includes('█'));
...
if (thresholdsIdx > 0 && thresholdsIdx < resultsIdx) {
  ...
  for (const line of thresholdLines) {
    if (line.includes('✓') || line.includes('✗')) {
      ...
      if (line.includes('✗')) thresholds[nameMatch[1]] = 'FAIL';
    }
  }
}
...
| 总请求数 | ${iterations} | - |
| 吞吐量 | ${httpReqs.match(/\s([\d.]+)\/s/)?.[1] || 'N/A'} req/s | - |

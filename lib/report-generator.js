...
if (meta?.plan && fs.existsSync(eventFile)) {
  const { records, issues: parseIssues } = await readRecords(eventFile, `${stem}.requests.jsonl`);
  ...
  if (!records.length) issues.push('没有请求明细，不能视为成功执行');
...
const overall = table(['指标', '结果', '口径'], [
  ...
  ['真实 TTFT', '未采集', 'k6 HTTP 缓冲响应；TTFB 不等于首字时间'],
...
const cohortRows = analysis?.cohorts.map((c, i) => [c.id, c.phase, n(c.issued), pct(c.successRate), c.failed, c.cancelled, c.unresolved,
  n(c.successLatency.p50), n(c.successLatency.p95), n(c.successLatency.p99), '未采集', JSON.stringify(c.errors), verdict(c, analysis.activity[i])]) || [];
...
${analysis ? table(['发起窗口', '模型', 'stream', '发起', '成功率', '成功 P95 ms', '输入字符 P50/P95', '已知输出 token P50/P95', '结束原因', '错误分布'], analysis.groups.map(g => [g.window, g.model, g.stream, g.issued, pct(g.successRate), n(g.successLatency.p95), `${n(g.inputChars.p50)}/${n(g.inputChars.p95)}`, `${n(g.outputTokenDistribution.p50)}/${n(g.outputTokenDistribution.p95)}`, JSON.stringify(g.finishReasons), JSON.stringify(g.errors)])) : '未采集分组明细。'}
...
- 首字测量附件：${cell(context.ttftEvidence)}。独立探针结果不代表本轮相同并发下的 TTFT。
